    ...
    try {
      await splitTransaction({
        ...
        splits: activeSplits.map((account) => {
          const amountValue = parseFloat(splitAmounts[account.id.toString()] ?? "0");
          return {
        ...
        <VStack spacing="lg" className="py-2">
              ...
              <VStack spacing="xs" align="start">
                ...
                <Muted size="xs">
                  {transaction.txDate?.seconds
                    ? new Date(Number(transaction.txDate.seconds) * 1000).toLocaleDateString()
                    : ""}
                </Muted>
              </VStack>
              <div className="text-sm font-mono font-semibold">
                {formatCurrency(formatAmount(transaction.txAmount), currencyCode)}
              </div>
            </HStack>
          </div>
          ...
          {friendAccounts.length === 0 ? (
            <VStack spacing="xs" align="center" className="py-4">
              ...
              <Muted size="xs">create a friend account first from the accounts page</Muted>
            </VStack>
          ) : (
            <VStack spacing="sm">
              <Caption>assign amounts</Caption>
              {friendAccounts.map((account) => (
                <HStack key={account.id.toString()} spacing="md" justify="between" align="center">
                  <div className="text-sm flex-1 truncate">
                    {account.friendlyName || account.name}
                  </div>
                  <HStack spacing="xs" align="center">
                    <Muted size="xs" className="font-mono">{currencyCode}</Muted>
                    <Input
                      type="number"
                      step="0.01"
                      min="0"
