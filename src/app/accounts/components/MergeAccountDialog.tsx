...
interface MergeAccountDialogProps {
  ...
  allAccounts: Account[];
  onConfirm: (primaryAccountId: bigint) => Promise<{ transactionsMoved: bigint }>;
}
...
export function MergeAccountDialog({
  ...
  const [transactionsMoved, setTransactionsMoved] = useState<bigint | null>(null);
  ...
  const selectedPrimary = mergeableAccounts.find((a) => a.id.toString() === selectedPrimaryId);
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <VStack spacing="md" className="py-4">
            <Muted size="sm">
              The account has been merged successfully.
            </Muted>
            <Card variant="subtle" padding="md">
              <VStack spacing="xs">
                <HStack spacing="md" justify="between">
                  <Text size="sm" weight="medium">transactions moved:</Text>
                  <Mono size="sm">{transactionsMoved.toString()}</Mono>
                </HStack>
                {selectedPrimary && (
                  <HStack spacing="md" justify="between">
                    <Text size="sm" weight="medium">surviving account:</Text>
                    <Mono size="sm">{selectedPrimary.friendlyName || selectedPrimary.name}</Mono>
                  </HStack>
                )}
              </VStack>
            </Card>
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <VStack spacing="md" className="py-4">
          <Muted size="sm">
            This account will be deleted and its transactions moved to the selected account.
          </Muted>
          <Card variant="subtle" padding="md">
            <VStack spacing="xs">
              <HStack spacing="md" justify="between">
                <Text size="sm" weight="medium">merging from:</Text>
              ...
              <SelectContent>
                {mergeableAccounts.map((a) => (
                  <SelectItem key={a.id.toString()} value={a.id.toString()}>
                    {a.friendlyName ? `${a.name} (${a.friendlyName})` : a.name} — {a.bank}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
